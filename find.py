...
def valid_next(chosen: list[RotatedCard], remaining: list[Card]):
  next_pos = len(chosen)
  row = next_pos // 3
  col = next_pos % 3
  ...
  if row == 0 and col == 0:
    ...
    return chosen[rc_to_idx(row-1, col)].find_valid_neighbors(2, remaining)
  else:
    # Two neighbors
    return find_valid(
...
def find_valid(chosen, remaining, target):
  ...
  return [
    find_valid(chosen + [candidate], remaining.difference(candidate.card))
